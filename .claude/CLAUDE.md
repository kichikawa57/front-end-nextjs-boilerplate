...
このプロジェクトは、機能ベースでコンポーネントとセクションを管理するディレクトリ構成を採用しています。

### ディレクトリ構造

```
app/
├── (pages)/                  # 機能ベースのコンポーネントとセクション（ルートではない）
│   └── home/                 # ホーム機能
│       ├── _components/      # ホーム専用のコンポーネント
│       │   └── HeroSection.tsx
│       └── _sections/        # ホームのセクション（大きなコンポーネントブロック）
│           └── MainSection.tsx
├── _components/              # 共通コンポーネント（複数機能で使用）
│   ├── Button.tsx
│   └── Header.tsx
...
#### 1. ページ: `app/page.tsx`, `app/about/page.tsx` など

実際のページは `app/` 直下またはルートディレクトリに配置します。

例:
- `app/page.tsx` → ルート: `/`
- `app/about/page.tsx` → ルート: `/about`
- `app/products/page.tsx` → ルート: `/products`

#### 2. 機能専用コンポーネント: `app/(pages)/[feature-name]/_components/`

**特定の機能に関連する**コンポーネントは `app/(pages)/[feature-name]/_components/` に配置します。

**ガイドライン:**
- 機能固有の小さな再利用可能な UI 要素
- 例: ヒーローセクション、フィーチャーカード、機能専用フォーム
- ページからインポートして使用できますが、機能ごとに整理されています
...
#### 3. 機能セクション: `app/(pages)/[feature-name]/_sections/`

複数のコンポーネントを組み合わせた大きなコンポーネントブロックは、機能の `_sections/` ディレクトリに配置します。
...
**複数の機能で使用される**コンポーネントは `app/_components/` ディレクトリに配置します。
...
1. **機能専用コンポーネントはページから自由にインポート可能**
   - `app/(pages)/home/_components/` 内のコンポーネントは任意のページから使用可能
   - 機能ごとに整理されているため、見つけやすく管理しやすい
...
3. **パス解決**
   ...
   - 機能専用コンポーネント: `@/app/(pages)/home/_components/HeroSection`
   - 機能セクション: `@/app/(pages)/home/_sections/MainSection`
